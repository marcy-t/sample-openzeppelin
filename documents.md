...
# Interacting programmatically ~ プログラムによるインタラクション
- コンソールはプロトタイピングや短髪のクエリやトランザクションを実行するのに便利です
- しかし、最終的には自分のコードからコントラクトを操作したい
- このセクションではJavascriptからコントラクトを操作する方法とHardhatを使用してHardhatの構成でスクリプトを実行する方法を行う

> 他にも多くのJavascriptライブラリがある
> コントラクトをデプロイした後はどのライブラリを通じてもコントラクトを操作することができます。

# [セットアップ](https://docs.openzeppelin.com/learn/deploying-and-interacting?pref=hardhat#setup)
- 新しいscripts/index.jsファイルでコーディングを始める
- 非同期コードを含むいくつかの定型文を初めJavascriptで記述
``` javascript
// script/index.js
...
```
- 有効なアカウントのリストなど、ローカルノードに何かを尋ねることでセットアップをテストすることが可能

- `npx hardhat node`
- `npx hardhat run --network localhost ./scripts/deploy.js`
- `npx hardhat run --network localhost ./scripts/index.js`
- npx hardhat nodeで立ち上げたアカウント一覧
``` bash
$ npx hardhat run --network localhost ./scripts/index.js
